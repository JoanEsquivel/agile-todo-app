...
- Dark mode is `@media (prefers-color-scheme: dark)` re-declaring `:root` token values in `tokens.css` — **no class toggle, no JS, no theme state anywhere.** If you need a new themed value, add a token and give it a dark-mode override; don't branch in a component.
